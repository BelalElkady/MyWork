...
extern u16 TIMER_u8SysTick(f32 Local_f32SetTick) {
	u32  Local_Timer;
	u16 NoOfOverFlow=0;

#if TIMER0_PRESCALER==FOSC_0
	Local_Timer=((Local_f32SetTick)*(F_CPU))/(1UL);
	if (Local_Timer > 255 ){
		NoOfOverFlow=((Local_f32SetTick)*(F_CPU))/(1UL*256UL);
	...
	Local_Timer=((Local_f32SetTick)*(F_CPU))/(8UL);
	if (Local_Timer > 255 ){
		NoOfOverFlow=((Local_f32SetTick)*(F_CPU))/(8UL*256UL);
	...
	Local_Timer=((Local_f32SetTick)*(F_CPU))/(1024UL);
	if (Local_Timer > 255 ){
		NoOfOverFlow=((Local_f32SetTick)*(F_CPU))/(1024UL*256UL);
